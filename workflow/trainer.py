...
class LlamaTrainer(SFTTrainer):
    ...
    def _inner_training_loop(
        ...
        total_params = kept_params = sum(
            [p.numel() if not p.requires_grad else 0 for p in model.parameters()]
        )
        # Train!
                ...
                if total_batched_samples % args.gradient_accumulation_steps == 0 or (
                    ...
                    if (
                        ...
                        if is_sagemaker_mp_enabled() and args.fp16:
                            grad_norm = self.optimizer.clip_master_grads(
                                args.max_grad_norm
                            )
                    ...
                    self._maybe_log_save_evaluate(
                        tr_loss,
                        grad_norm if grad_norm is not None else None,
                        model,
                        trial,
                        epoch,
...
def get_training_args(training_configs, model_name):
...
def get_trainer(
    ...
    training_args = get_training_args(training_configs, model_name)
    if model_name in ["bert-base-cased"]:
        trainer = BertTrainer(
            model=local_model,
            train_dataset=train_dataset,
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            training_configs.base_model_path, padding_side="right"
        )
        ...
        # print(f"tokenizer.eos_token: {tokenizer.pad_token}")
    return trainer
